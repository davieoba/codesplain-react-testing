...
function RepositoriesListItem({ repository }) {
  ...
  return (
    <div className="py-3 border-b flex">
      {/* this FileIcon component has an aria-label */}
      <FileIcon name={language} className="shrink w-6 pt-1" />
      <div>
        <Link to={`/repositories/${full_name}`} data-testid='editor-link' className="text-xl">
      ...
      <div className='grow flex items-center justify-end pr-2'>
        <a href={repository.html_url} data-testid='repo-link' aria-label='github repository'>
          <MarkGithubIcon />
        </a>
      </div>
    </div>
